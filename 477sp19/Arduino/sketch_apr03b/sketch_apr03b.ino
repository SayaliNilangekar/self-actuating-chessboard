...
void setup(void){
  ...
  wifiMulti.addAP("Winternet is coming", "Password");   // add Wi-Fi networks you want to connect to
  ...
  server.on("/goback", HTTP_POST, handleGoBack); // Call the 'handleGoBack' function when a POST request is made to URI "/goback"
...
void handleRoot() {                          // When URI / is requested, send a web page with a button to toggle the LED
  ...
  server.send(200, "text/html", "<p>Hello!</p><p>Enter value here:</p><form action=\"/submit\" method=\"POST\"><input type=\"text\" name=\"move\" placeholder=\"Move\"></br><input type=\"submit\" value=\"Submit\"></form>");
...
void handleSubmit() {                         // If a POST request is made to URI /login
    //server.send(200, "text/html", "<p>Got it!<\p>");
    server.send(200, "text/html", "<p>Got it!</p><form action=\"/goback\" method=\"POST\"><input type=\"submit\" value=\"Go back\"></form>");
}
